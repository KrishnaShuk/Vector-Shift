...
const store = (set, get) => ({
  ...
  addNode: (node) => {
    ...
    const baseName = type.replace("custom", "").toLowerCase();
      ...
      edges: addEdge(
        {
          ...
          type: "custom",
          animated: true,
          markerEnd: { type: MarkerType.Arrow, height: "15px", width: "15px" },
        },
        get().edges
      ),
    }),
  ...
  updateTextNodeAndSyncEdges: (textNodeId, newText) => {
    ...
    const variablesInText = new Set(
      Array.from(newText.matchAll(variableRegex), (match) => match[1].trim())
    );
...
export const useStore = create(
  persist(store, { name: "vectorshift-pipeline-storage" })
);
